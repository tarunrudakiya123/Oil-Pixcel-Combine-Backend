...
class OrderController {
  async CreateOrder(req, res) {
    try {
      const { products, paymentMethod, shippingAddress, userInfo, totalPrice } =
        req.body;

      if (!products)
        return res
          .status(400)
          .send({ message: "Missing Dipendecy of Products" });

      if (!paymentMethod)
        return res
          .status(500)
          .send({ message: "Missing Dipendecy of PaymentMethods" });

      if (!shippingAddress)
        return res
          .status(500)
          .send({ message: "Missing Dipendecy of ShppingAddress" });

      if (!userInfo)
        return res
          .status(500)
          .send({ message: "Missing Dipendecy of UserInfo" });
  ...
  async PaymentVerify(req, res) {
    ...
    try {
      ...
      if (
        (response.status === "captured" || response.status === "authorized") &&
        response.order_id === razorpayOrderId
      ) {
