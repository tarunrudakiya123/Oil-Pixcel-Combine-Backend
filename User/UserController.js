...
class UserController {
      ...
      if (error && error.message && error.message.includes("E11000")) {
        return res
          ...
          .send({
            message: "Validation Error",
            ValidationResult: [
              { key: "email", message: "Email Is Alredy Exist" },
            ],
          });
      }
  ...
  async UserLogin(req, res) {
    try {
      ...
      if (!email)
        return res.status(400).send({ message: "Missing Dipendancy Email" });
      ...
      if (!bcrypt.compareSync(password, user.password)) {
        return res
          ...
          .send({
            message: "Validation Error",
            ValidationResult: [
              { key: "password", message: "Email And password is Not Match" },
            ],
          });
      }
